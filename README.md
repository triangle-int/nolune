...
| `nolune config show` / `set` / `unset` / `email add` / `email remove` | Shows or changes the companion's timezone, name, API keys, GitHub token, and SMTP/IMAP accounts in the files the Settings page writes; the server picks them up on the next message. `show` never prints secret values, and secrets (API keys, the token, email passwords) are read from stdin only, never from arguments. The companion changes its own settings with this command through its built-in `configure-nolune` skill |
