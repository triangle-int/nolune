...
| `nolune cua install` | Downloads the Cua Driver release this version of Nolune is pinned to, verifies its size and sha256 before anything is written, installs it under `~/.nolune/cua-driver/`, and refuses a driver that reports any other version. `--force` reinstalls; `NOLUNE_CUA_RELEASE_URL` names a mirror |
| `nolune cua status` | Shows the pinned version, whether this host is supported (macOS; Linux and Windows report `unsupported`, a host without a display reports `headless`), the installed and the discovered driver, its version against the pin, and its health and permission state |
