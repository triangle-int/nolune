<script lang="ts">
	...
	// and points at the Allow and Deny of its approval above; a settled one
	// says what happened. The two quoted labels are the peer's own words
	// (who it speaks for, why it asks), unverified; nothing a peer sent
	// beyond them is ever shown here, because the server keeps no text.
		...
		{#if needing.length > 0}
			<ul class="inbox-list" aria-label="Requests from companions waiting for you">
				{#each needing as row (row.key)}
					<li class="inbox-row" data-status={row.status}>
...
<style>
	...
	.inbox-row[data-status="pending"] { background: var(--accent); border-color: var(--primary); }
	...
	.inbox-row[data-status="pending"] .inbox-state { color: var(--primary); border-color: var(--primary); }
	.inbox-row[data-status="denied"] .inbox-state { color: var(--destructive); border-color: var(--destructive); }
