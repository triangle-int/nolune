...
async fn build_turn_context(
    ...
    // Instance config — serialize the whole struct so new fields are
    // automatically visible. The token itself is never the model's to read.
    let mut shown_cfg = instance_cfg.clone();
    if !shown_cfg.github.token.is_empty() {
        shown_cfg.github.token = "(set)".into();
    }
...
mod prompt_stability_tests {
    ...
    async fn the_turn_context_never_carries_the_github_token() {
        ...
        assert!(context.contains("token = \"(set)\""), "{context}");
    }
