...
<section id="install" class="install"><div class="section-shell"><div class="install-grid"><Reveal><div><p class="eyebrow">Native installation</p><h2 class="section-title">A home for Nolune,<br />on hardware you own.</h2><p class="section-copy">Install the server on macOS or Linux with one command, or straight from the desktop app, then connect every computer you want Nolune to use.</p><div class="facts"><span>OPEN SOURCE</span><span>MIT LICENSED</span><span>BYOK</span></div></div></Reveal><Reveal delay={100}><div class="terminal"><div class="terminal-head"><span>TERMINAL · INSTALL SERVER</span><i>macOS / Linux</i></div><div class="command"><span>$</span><code>{command}</code><button onclick={copyCommand} aria-label="Copy install command">{copied ? 'COPIED' : 'COPY'}</button></div><p class="copy-status" aria-live="polite" aria-atomic="true">{copyStatus}</p><div class="output"><p><b>01</b> Downloads the native server</p><p><b>02</b> Prepares your workspace and token</p><p><b>03</b> Starts the server and opens localhost:26559</p></div><a class="release" href="https://github.com/triangle-int/nolune/releases" target="_blank" rel="noopener"><span>DESKTOP APPS</span><strong>Download for macOS, Windows, or Linux</strong><i>↗</i></a></div></Reveal></div></div></section>
<style>
