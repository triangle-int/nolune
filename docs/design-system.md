...
Little Moon reflects what Nolune is really doing, never a decorative mood. The state comes from the pure reducer in `client/src/lib/companion/state.js`, held by the scene store (`scene.companion`, `scene.companionStatus`) and fed from runtime events only: the root layout passes every websocket event through `companionEventFromServer` (and the answered secret request), and `ChatView` adds the persisted `agent_running` of each conversation snapshot it loads, so a run the socket missed still shows and one that ended while the client was away is over without being claimed. Every state has an accessible sentence that names the related action, computer, request, or blocker, so it reads without motion; when several facts hold at once the highest row wins.
...
| 3 | Waiting for approval | An unanswered `secret_request` or approval | “Nolune is waiting for you: a GitHub token for gh.” | `waiting.svg` (glancing up at you) · hold (soft pulse) |
| 4 | Failed | The server’s `[system] <error>` assistant message that precedes `agent_stopped` (which carries no error field), an `agent_stopped` that names one, or a proactive run’s failed receipt | “Nolune stopped with an error: something went wrong.” | `failed.svg` (eyes down) · still |
