...
- Codex login (#27): the Codex section of Settings › Connections is one tile (`lib/components/settings/CodexLogin.svelte`, copy from `lib/models/codex.js`): a row like an API key's with the name, the release and its path in mono under it, a lavender badge (Logged in, Waiting for you) and the link-style actions (Log in in the add color, Use a device code muted, Log out or Cancel in the destructive color); under the row one 14px state line and, when there is something to do, a secondary-text sentence in `--primary`. A pending login reuses the pairing panel: the URL as a mono link, the device code in the pairing-code size, and a note that the page updates itself. Errors are inline `--destructive` text with `role="alert"`. Onboarding offers Codex as a fourth provider pill (two columns) and shows the same URL and code in the pill grid's place; while the browser flow waits, a hint-style link under the panel ("on another device? use a device code") swaps it for a device code, above "choose another provider". `/design-system` renders the pending, logged-in and not-installed states from sample statuses.
...
- Settings: permissions are a bordered list on `--card`; a granted state is a text badge, an ungranted state is a muted state badge (Denied, Not asked yet) beside a lavender Grant button. The rows are the Cua Driver's own grants (`com.trycua.driver`), introduced by a lead that says so and that every screenshot is a one-shot snapshot during an action; status lines above them (driver install, version against the pin, health) use secondary text for what is fine, muted for what does not apply (Linux, Windows, headless), and `--destructive` text with `role="alert"` for what needs doing, always with the command to run. Failed status checks show an error and Retry, never an empty list. This app's own grants, for the older coordinate tools, follow as a smaller secondary list.
