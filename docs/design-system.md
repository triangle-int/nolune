...
- Codex login (#27): the Codex section of Settings › Connections is one tile (`lib/components/settings/CodexLogin.svelte`, copy from `lib/models/codex.js`): a row like an API key's with the name, the release and its path in mono under it, a lavender badge (Logged in, Waiting for you) and the link-style actions (Log in and Use a device code in the add color, Log out or Cancel in the destructive color); under the row one 14px state line and, when there is something to do, a secondary-text sentence in `--primary`. A pending login reuses the pairing panel: the URL as a mono link, the device code in the pairing-code size, and a note that the page updates itself. Errors are inline `--destructive` text with `role="alert"`. Onboarding offers Codex as a fourth provider pill (two columns) and shows the same URL and code in the pill grid's place. `/design-system` renders the pending, logged-in and not-installed states from sample statuses.
