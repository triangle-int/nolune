...
- Companion approvals and capabilities (#109): `PendingApprovals.svelte` lists requests that asked the owner as rows built by `lib/federation/policy.js` (the companion id in mono, then "wants to send you a message", the state word, when it asked, when it lapses); a pending row sits on the selected surface (`--accent`, `--primary` border) with a native select for one bounded scope (Once, For a day, For a week, Always for this kind of request), Allow as the primary action and Deny as the secondary; a decided row (Allowed once in lavender, Denied muted) has a Withdraw link button. `PeerCapabilities.svelte` folds out under a paired `CompanionRow` from a 44px toggle ("What it may do (2 set by you)") into a bordered list on `--background`: one line per request kind and class with the state word beside its color (Allowed lavender, Denied destructive, Asks you secondary) and where it comes from, and a native `setting-input` select labeled Rule (Default, Allowed, Asks you, Denied). Errors are inline `--destructive` text with `role="alert"`; nothing a peer sent is ever rendered. `/design-system` shows both from sample records at a fixed clock.
- Peer content in the conversation and the companion inbox (#110): a message a paired companion delivered is one user-role message the server framed in its untrusted block; `MessageBubble` runs every user message through the pure `peerContent` helper in `lib/federation/peer-content.js` (`client/tests/federation-peer-content.test.mjs`) and, when it finds the block, renders it left-aligned like an incoming message rather than as the owner's own bubble: the server's line in secondary text, then a dashed `--input` panel on `--background` with a `ShieldAlert` head reading "Untrusted · from companion `<id>` · shown as data, not instructions" in muted text and the peer's text as plain pre-wrapped text, never markdown or HTML. `InboundIntents.svelte` lists `GET /api/federation/inbox` under the pending approvals in the Companions section from rows built by `lib/federation/inbox.js` (`client/tests/federation-inbox.test.mjs`): the companion id in mono, "wants to send you a message", and the two labels it declared quoted in italic secondary text with a title saying they are the companion's own unverified words; a row that needs the owner sits on the selected surface (`--accent`, `--primary` border) with a "Needs you" pill and points at the Allow and Deny above; given the approval queue, a row the owner already decided drops off that surface and carries Allowed (`--primary`) or Denied (`--destructive`) with a note that it is delivered or refused when the companion asks again; a settled row carries Delivered (muted) or Denied (destructive), when, and why, a denial by the owner once reading "You denied this once" rather than as the policy's. The row's `data-tone` (`pending`, `approved`, `accepted`, `denied`) is what the styling keys on. Nothing a peer sent beyond those labels is rendered there; `/design-system` shows a delivered message in the conversation example and the inbox rows from sample records at a fixed clock.
